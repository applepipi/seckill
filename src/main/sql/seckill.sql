...
START TRANSACTION; I
NSERT ignore INTO success_killed (seckill_id, user_phone, create_time) VALUES(v_seckill_id, v_phone, v_kill_time);
SELECT ROW_COUNT() INTO insert_count;
IF (insert_count = 0) THEN ROLLBACK; SET r_result = -1;
...
DELIMITER;
set @r_result=-3;
call execute_seckill(1003,15764545454,now(),@r_result);
select @r_result;


--存储过程优化：事务行级锁持有的时间
--不要过度依赖存储过程(银行中用的多，直接将业务逻辑打包到存储过程，但别的互联网公司不太用)
--简单的逻辑可以应用存储过程
--QPS：一个秒杀单6000/qps